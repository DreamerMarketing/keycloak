...
| `delegation`
| Extends the `username` type with an additional impersonation check. The parameter must be the username (or email address when the realm allows login with email) of an existing, enabled user. The user cannot target themselves, and the current user must have impersonation permissions for the target user.
| No
